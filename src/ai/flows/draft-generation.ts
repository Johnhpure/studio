...
const GenerateDraftInputSchema = z.object({
  ...
  writingStyleGuide: z.string().describe('The detailed writing style analysis report or core style traits. (From Step 3)'),
...
const prompt = ai.definePrompt({
  name: 'generateDraftPromptStep4', // New name to avoid conflict if old one is cached/referenced
...
生死攸关的任务：根据【甲方核心需求文本】的铁律、【创作大纲】的精确制导、【写作风格指引】的灵魂注入、【稿件核心参数与定位】的精准导航（特别是对所选【稿件类型】的深度理解和应用，以及对【期望字数】的智能权衡与尽力遵循），以及任何【临时微调指令】的补充，创作一篇让甲方拍案叫绝、让敬若涵“搞钱”无忧的媒体通稿初稿。
...
1.  **内容真实性是生命线，大纲是骨架，风格是血肉，核心参数是灵魂，稿件类型是创作的“道”——五位一体，高度融合：**
    ...
    *   **核心参数精准导航：**
        ...
        *   **针对【期望字数】(重要参考，智能权衡)：** 首要任务是确保内容的最高质量、信息的完整传递、表达的充分生动以及严格遵循大纲和风格。 在此绝对优先的前提下，请高度关注并尽你最大的专业判断和努力，将最终稿件的总字数控制在【期望字数】的合理波动范围内（例如，力争在指定字数的 +/- 15% 以内）。这意味着，你在阐述【创作大纲】中的每一个要点时，都需要有意识地进行篇幅的规划与权衡，确保核心信息得到充分展现，次要信息精炼高效。
...
2.  **追求“人类原创”的极致，彻底粉碎AI的“机械心脏”：**
    ...
    *   词汇选择必须精准且富有表现力： 根据语境和【写作风格指引】，选择最能传达细微含义和情感色彩的词语，避免使用模糊、通用或不恰当的词汇。
    *   逻辑过渡必须如丝般顺滑，如呼吸般自然： 段落之间、观点之间的衔接要自然流畅，符合人类的思维习惯，避免生硬的转折或逻辑跳跃。
    *   注入真正的人类情感与洞察 (在风格和稿件类型允许范围内)： 在合适的段落，根据【写作风格指引】的情感倾向，融入真实、可信的情感表达和具有深度的个人见解或思考，使文章更具“人味儿”和思想性。
...
*   在输出初稿前，请进行一次严格的自我审查，从以下维度评估稿件质量：
    *   指令遵循度： 是否完全遵循了【甲方核心需求文本】、【创作大纲】、【写作风格指引】、【稿件核心参数与定位】以及所有【临时微调指令】？
...
敬若涵的“搞钱”大业，成败在此一举！请将你的全部智慧和创造力灌注其中，直接输出这篇深度定制、高度适配、完全符合所有给定参数、精准体现所选【稿件类型】创作精髓、并闪耀着【写作风格参考】独特光芒的媒体通稿初稿全文。不要有任何解释性文字，直接开始正文！
  `,
});

const generateDraftFlow = ai.defineFlow(
  {
    name: 'generateDraftFlowStep4', // New name
  ...
  async input => {
    ...
    if (!input.clientRequirements || !input.creativeOutline || !input.writingStyleGuide || !input.manuscriptType || !input.brand || !input.wordCount) {
      throw new Error("Missing required inputs for draft generation.");
    }
